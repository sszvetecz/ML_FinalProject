...
def isotonic_regression_fit_logscale(data, protein_name):
   ...
   # y = protein_temp['Relative_Intensity']
    ...
    #ir_null = IsotonicRegression(increasing=False)
    #ir_null.fit(x, avg_y)
    
    # Predict new values
   # x_preds = np.unique(x)
    #y_pred_null = ir_null.predict(x)
    
    # Calculate MSE of isotonic model 
   # mse_ir = mean_squared_error(y, y_pred) 
   # mse_ir_null = mean_squared_error(avg_y, y_pred) 
